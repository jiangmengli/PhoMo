...
def cgp_edge_main_process(chemdraw_file_list_type_conjugated, chemdraw_file_list_stereo,
    ...
    if not os.path.isdir('./outputs/'):
        os.mkdir('./outputs/')
    # prune the molecule graph -----------------------------------------------------------------------------------------
    dir_list = os.listdir('./inputs/' + chemdraw_record_version + '/graphs/causality_reasoning/')
    sort_list = []
    for i in range(len(dir_list)):
        if dir_list[i].startswith('drop_'):
            sort_list.append(int(dir_list[i].split('_')[-1]))
    sort_list = sorted(sort_list)
    dir_list = []
    for i in range(len(sort_list)):
        dir_list.append('drop_' + str(sort_list[i]))
    for prune_dir in dir_list:
        print('Process: Pruning process -> ' + str(prune_dir))
        # Read the data
        edge_list_new = copy.deepcopy(edge_list)
        stereo_edge_list_new = copy.deepcopy(stereo_edge_list)
        # Pruning
        prune_id = int(prune_dir.split('_')[-1].strip())
        for i in range(len(edge_list_new)):
            if prune_id < len(edge_list_new[i]):
                del edge_list_new[i][prune_id]
                del stereo_edge_list_new[i][prune_id]
        # Generate molecule fingerprints
        generate_moleccule_fingerprints(experiment_file, chemdraw_record_version, edge_list_new, edge_type_dict,
                                        stereo_edge_list_new, stereo_edge_type_dict, molecule_list,
                                        stereo_molecule_list, bad_molecule_list, bad_edge_list, prune_dir)
    return True


def cgp_node_main_process(chemdraw_file_list_type_conjugated, chemdraw_file_list_stereo,
                          bad_chemdraw_file_list_type_conjugated, bad_chemdraw_file_list_stereo,
                          experiment_file, chemdraw_record_version):
    # For positive samples -----------------------
    # Get data
    ...
    for prune_dir in dir_list:
        print('Process: Pruning process -> ' + str(prune_dir))
        # Read the data
        molecule_list_new = copy.deepcopy(molecule_list)
        stereo_molecule_list_new = copy.deepcopy(stereo_molecule_list)
        edge_list_new = copy.deepcopy(edge_list)
        stereo_edge_list_new = copy.deepcopy(stereo_edge_list)
        # bad_molecule_list_new = copy.deepcopy(bad_molecule_list)
        # bad_edge_list_new = copy.deepcopy(bad_edge_list)
        # Pruning
        prune_id = int(prune_dir.split('_')[-1].strip())
        for i in range(len(molecule_list_new)):
            if prune_id < len(molecule_list_new[i]):
                del molecule_list_new[i][prune_id]
                del stereo_molecule_list_new[i][prune_id]
                del_node = prune_id + 1
                j = 0
                while j < len(edge_list_new[i]):
                    if int(edge_list_new[i][j]['head']) == del_node or int(edge_list_new[i][j]['tail']) == del_node:
                        del edge_list_new[i][j]
                        del stereo_edge_list_new[i][j]
                    else:
                        if int(edge_list_new[i][j]['head']) > del_node:
                            edge_list_new[i][j]['head'] -= 1
                        if int(edge_list_new[i][j]['tail']) > del_node:
                            edge_list_new[i][j]['tail'] -= 1
                        j += 1
        # Generate molecule fingerprints
        generate_moleccule_fingerprints(experiment_file, chemdraw_record_version, edge_list_new, edge_type_dict,
                                        stereo_edge_list_new, stereo_edge_type_dict, molecule_list_new,
                                        stereo_molecule_list_new, bad_molecule_list, bad_edge_list, prune_dir)
    return True


def generate_moleccule_fingerprints(experiment_file, chemdraw_record_version, edge_list, edge_type_dict,
                                    stereo_edge_list, stereo_edge_type_dict, molecule_list, stereo_molecule_list,
                                    bad_molecule_list, bad_edge_list, prune_dir):
    # molecule oriented dataset ----------------------------------------------------------------------------------------
    print('Process: Molecule Oriented Dataset ...')
    output_path = './outputs/' + chemdraw_record_version + '/'
    if not os.path.isdir(output_path):
        os.mkdir(output_path)
    output_path += prune_dir + '/'
    if not os.path.isdir(output_path):
        os.mkdir(output_path)
    ...
    output_path = './outputs/' + chemdraw_record_version + '/' + prune_dir + '/experiment_oriented/'
    if not os.path.isdir(output_path):
        os.mkdir(output_path)
...
if __name__ == '__main__':
    # Setting
    chemdraw_record_version = config_chemdraw_causal_version
    ...
    # Main processes -----------------------------------
    if config_chemdraw_causal_version.startswith('CauReaEdge'):
        if cgp_edge_main_process(chemdraw_file_list_type_conjugated, chemdraw_file_list_stereo,
                                 bad_chemdraw_file_list_type_conjugated, bad_chemdraw_file_list_stereo,
                                 experiment_file, chemdraw_record_version):
            print('CauReaEdge Process: ChemDraw_Graph_Processing Finished')
        else:
            print('CauReaEdge Error: ChemDraw_Graph_Processing Failed')
    elif config_chemdraw_causal_version.startswith('CauReaNode'):
        if cgp_node_main_process(chemdraw_file_list_type_conjugated, chemdraw_file_list_stereo,
                                 bad_chemdraw_file_list_type_conjugated, bad_chemdraw_file_list_stereo,
                                 experiment_file, chemdraw_record_version):
            print('CauReaNode Process: ChemDraw_Graph_Processing Finished')
        else:
            print('CauReaNode Error: ChemDraw_Graph_Processing Failed')
    else:
        print('\'config_chemdraw_causal_version\' is NOT correct!')
