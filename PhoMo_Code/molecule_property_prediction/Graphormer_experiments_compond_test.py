...
if __name__ == '__main__':
    ...
    print("======= pre process ! =======\n")
    sample_total_num = 364 # 这里填写样本数量总和
    # 将./PC_experiment_json.json改为当前的，即原始的PC_experiment_json.json（在上一级目录ChemGraph里面）加上新的PC_experiment_json.json
    # merge csvs
    Merge_csvs.merge_csvs(path1 = './dataset/cuihuaji_uncompressed', path2 = './dataset/cuihuaji_newfinded_uncompressed2')
    ...
    cmd = "mv ./dataset/cuihuaji/split/scaffold/test.csv.gz ./dataset/cuihuaji/split/scaffold/test.csv.gz.backup"
    os.system(cmd)

    cmd = "mv ./dataset/cuihuaji/split/scaffold/train.csv.gz ./dataset/cuihuaji/split/scaffold/train.csv.gz.backup"
    os.system(cmd)

    cmd = "mv ./dataset/cuihuaji/split/scaffold/valid.csv.gz ./dataset/cuihuaji/split/scaffold/valid.csv.gz.backup"
    os.system(cmd)

    cmd = "cp ./dataset/cuihuaji_merged/split/* ./dataset/cuihuaji/split/scaffold/"
    os.system(cmd)
    ...
    cmd = "cp -r  ./dataset/cuihuaji_merged/* ./dataset/cuihuaji/raw/"
    os.system(cmd)

    # cmd = "cp ./dataset/cuihuaji_newfinded_uncompressed2/PC_experiment_json.json ./PC_experiment_json.json"
    # os.system(cmd)

    print("======= step 0.5: get graph matching outputs =======\n")

    skip = True

    if not skip:
        cmd = "cp -r ./dataset/cuihuaji_merged/* .//GIN/GIN-PCQimp/matching"
        os.system(cmd)
    ...
    if not skip:
        ...
        cmd = "rm ./Graphormer-main/graphormer/graphormer_output_np.npy"
        print(cmd)
        os.system(cmd)

        cmd = " cd ./Graphormer-main/graphormer && /home/iscas/miniconda3/envs/gcl/bin/python ./entry.py --num_workers 8 --seed 1 --batch_size 364 --dataset_name cuihuaji --accelerator cpu --precision 32 --ffn_dim 768 " + \
        ...
        os.system(cmd)
        # 在此处进行最终结果的计算
        l, p = feature_test_judge_new.train("./graphormer_output_np_ori_test.npy", class_num='multi', predition='TON', MLF=True, sample_total_num=364)
        ...
        for i in range(10):
            ...
            cmd = "cd ./Graphormer-main/graphormer && /home/iscas/miniconda3/envs/gcl/bin/python ./entry.py --num_workers 8 --seed 1 --batch_size 363 --dataset_name cuihuaji --accelerator cpu --precision 32 --ffn_dim 768 " + \
                " --hidden_dim 768 --intput_dropout_rate 0.0 --attention_dropout_rate 0.3 --dropout_rate 0.1 --weight_decay 0.01 " + \
                "--n_layers 12 --multi_hop_max_dist 5 --default_root_dir ./Graphormer-main/graphormer --tot_updates 1000" + \
                " --warmup_updates 3417 --max_epochs 1 --peak_lr 2e-4 " + \
                "--checkpoint_path /home/iscas/GRL/ChemGraph/ChemGraph/WholeProcess2/Graphormer-main/model/PCQM4M-LSC-epoch=424-valid_mae=0.1277.ckpt " + \
            ...
            print(cmd)
            os.system(cmd)
            l, p = feature_test_judge_new.train("./graphormer_output_np_ori_test.npy", class_num='binary', predition='TON', MLF=True)
            p_all = p_all + p
        p_avg = p_all.mean(dim=1)
        accs = np.array(accs)
        print("=== result === \n graphormer MLF test acc is ", accs.mean(), " std is ", accs.std())

        print("clean split")

        cmd = "rm ./dataset/cuihuaji/split/scaffold/*.gz"
        os.system(cmd)

        cmd = "mv ./dataset/cuihuaji/split/scaffold/test.csv.gz.backup ./dataset/cuihuaji/split/scaffold/test.csv.gz"
        os.system(cmd)

        cmd = "mv ./dataset/cuihuaji/split/scaffold/train.csv.gz.backup ./dataset/cuihuaji/split/scaffold/train.csv.gz"
        os.system(cmd)

        cmd = "mv ./dataset/cuihuaji/split/scaffold/valid.csv.gz.backup ./dataset/cuihuaji/split/scaffold/valid.csv.gz"
        os.system(cmd)
        ...
        for i in range(5):
            ...
            print(cmd)
            os.system(cmd)
            accs.append(feature_test.train("./graphormer_output_np_ori_test.npy", class_num='binary', MLF=False))
        accs = np.array(accs)
        print("=== result === \n graphormer no MLF test acc is ", accs.mean(), " std is ", accs.std())
        ...
        #     print(cmd)
        #     os.system(cmd)
        #     accs.append(feature_test.train("./graphormer_output_np_ori_test.npy", class_num='multi', predition='CAC', MLF=True))
        # accs = np.array(accs)
        # print("=== result === \n graphormer CAC MLF test acc is ", accs.mean(), " std is ", accs.std())
        #
        # accs = []
        # for i in range(5):
        #     # cmd = "rm ./Graphormer-main/graphormer/graphormer_output_np_drop1000.npy"
        ...
        #     print(cmd)
        #     os.system(cmd)
        #     accs.append(feature_test.train("./graphormer_output_np_ori_test.npy", class_num='multi', predition='CAC', MLF=False))
        # accs = np.array(accs)
        # print("=== result === \n graphormer CAC no MLF test acc is ", accs.mean(), " std is ", accs.std())
