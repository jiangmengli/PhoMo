...
def main():
    ...
    dataset_name = 'cuihuaji'
    file_path = '../dataset/' + dataset_name + '/master.csv'
    print(file_path)
    master = pd.read_csv(file_path, index_col=0)
    meta_dict = master[dataset_name]
    meta_dict['dir_path'] = '../dataset/' + dataset_name
    dataset_test = {
        'num_class': 128,  # to change
        'loss_fn': F.binary_cross_entropy_with_logits,  # to change
        'metric': 'ap',  # to change
        'metric_mode': 'max',  # to change
        'evaluator': ogb.graphproppred.Evaluator('ogbg-molpcba'),  # to change
        'dataset': MyGraphPropPredDataset('cuihuaji', root='../dataset', meta_dict=meta_dict),
        'max_node': 128,
    }
    ...
    # dataset_test = get_dataset(self.dataset_name)
    ...
    #     pin_memory=False,
    #     collate_fn=partial(collator, max_node=dataset_test['max_node']),
    # )
