...
class SIGMA(nn.Module):
    ...
    def forward(self, x_s, edge_index_s, x_t, edge_index_t, T, edge_attr_s, edge_attr_t, miss_match_value=-1.0):
        loss = 0.0
        loss_count = 0
        ...
        for _ in range(T):
            ...
            # propagate
            h_s = self.f_update(x_s, x_s_importance, edge_index_s, edge_attr_s)
            h_t = self.f_update(x_t, x_t_importance, edge_index_t, edge_attr_t)
        ...
        # print(" best_reward is ", best_reward, " loss is ", loss / float(loss_count))

        return best_reward, loss / float(loss_count)
