...
class Graphormer(pl.LightningModule):
    def __init__(
        ...
        # if dataset_name == 'PCQM4M-LSC' or dataset_name == 'ZINC':
        ...
        a = get_dataset(dataset_name)['loss_fn']
        ...
        if dataset_name == 'cuihuaji':
            self.cuihuaji_learnepmdata = nn.Linear(6, hidden_dim)
            self.cuihuaji_projecthead = nn.Linear(hidden_dim * 2, 8)
            self.cuihuaji_loss = nn.MSELoss( reduce = True, size_average = True)
    ...
    def forward(self, batched_data, perturb=None):
        ...
        output_np = output.detach()
        output_np = output_np.cpu()
        output_np = output_np.numpy()
        np.save(f'./graphormer_output_np.npy', output_np[:, 0, :])

        # output part
        if self.dataset_name == 'PCQM4M-LSC':
            # get whole graph rep
            output = self.out_proj(output[:, 0, :])
        elif self.dataset_name == 'cuihuaji':
            number, epm_data, _, _ = batched_data.y.split([1, 6, 21, 1], dim=1)
            epm_output = self.cuihuaji_learnepmdata(epm_data.float())
            mol_output = output
            output = torch.cat((epm_output, mol_output[:, 0, :]), 1)
            output = self.cuihuaji_projecthead(output)
        else:
        ...
        output_np = output_np.cpu()
        output_np = output_np.numpy()
        np.save(f'./graphormer_output_np.npy', output_np)
        # np.save(f'./graphormer_output_np_drop' + str(self.tot_updates), output_np)

        print("\n=== output.shape[0] is ===",output.shape[0])
        
        if output.shape[0] == 334:
            sys.exit()
    ...
    def training_step(self, batched_data, batch_idx):
        if self.dataset_name == 'ogbg-molpcba':
            ...
            # get features
            features = self(batched_data)

            y_hat = self(batched_data).view(-1)
            y_gt = batched_data.y.view(-1)
            loss = self.cuihuaji_loss(y_hat, y_gt)
            # loss = 1
        else:
            y_hat = self(batched_data).view(-1)
            y_gt = batched_data.y.view(-1)
            loss = self.loss_fn(y_hat, y_gt)
        print("model.py 237 train_loss:", loss)
        return loss
